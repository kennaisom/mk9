...
class crf_basic_options
{
		...
		public function crf_null_field_notice() 
		{
			...
			<div class="notice" style="border-left:4px solid #ffd802">
				<p><?php _e( 'Some of the options below require selecting fields from your form. Since you are creating new form from scratch, there are no fields in this form yet. You can come back later and modify these field specific options. You can safely ignore them for now and save the settings.', $textdomain ); ?></p>
			</div>
			<?php
		}
		...
		public function add_crf_fields_list() 
		{
			...
			?>
    		 <select id="autoresponder_fields" onChange="add_field_autoresponder(this.value)"> 
       		<?php $this->crf_fields_dropdown_options($id,'');?>
        	</select>
            <?php
		}
			 ...
			 foreach($reg1 as $row1)
			 {
				if(!empty($row1))
				{
					...
					{	
						...
						{
							$value = rtrim(implode(',',$post[$Customfield]),',');	
						}
						else
						{
		...
		public function crf_send_user_email($id,$post,$userid=0,$entry_id)
		{
				global $wpdb;
				$crf_fields=$wpdb->prefix."crf_fields";
			  ...
			  //$message = $this->crf_get_form_option_value('crf_welcome_email_message',$id);
		...
		public function crf_get_subscriber_other_field($id,$post,$field)
		{
		...
		public function crf_get_subscriber_email($id,$post)
		{
				global $wpdb;
				$crf_fields = $wpdb->prefix."crf_fields";
				$textdomain = 'custom-registration-form-builder-with-submission-manager';
		...
		public function crf_get_submissions($entries,$form_id)
		{
			global $wpdb;
			$crf_submissions =$wpdb->prefix."crf_submissions";	
			$crf_fields =$wpdb->prefix."crf_fields";	
