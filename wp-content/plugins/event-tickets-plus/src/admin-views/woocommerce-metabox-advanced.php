...
<?php if ( $this->supports_global_stock() ): ?>
	<tr class="<?php $this->tr_class(); ?> global-stock-mode">
		<td><label for="ticket_woo_global_stock"><?php esc_html_e( 'Global stock mode:', 'event-tickets-plus' ); ?></label></td>
		<td>
	...
	<tr class="<?php $this->tr_class(); ?> global-stock-mode sales-cap-field">
		<td><label for="ticket_woo_global_stock_cap"><?php esc_html_e( 'Cap sales:', 'event-tickets-plus' ); ?></label></td>
		<td>
	...
	<tr class="<?php $this->tr_class(); ?>">
		<td colspan="2" class="tribe_sectionheader updated">
			...
			<div class="bumpdown" data-trigger="selling-tickets-info">
				<?php
					if ( is_admin() ) {
						esc_html_e( 'Currently, WooTickets will only show up on the frontend once per full event. For PRO users this means the same ticket will appear across all events in the series. Please configure your events accordingly.', 'event-tickets-plus' );
					} else {
